...
### Mark the chosen path without deleting the others
...
### Generate a visual map when useful
Use `scripts/branch_map_svg.py` to render a simple SVG from a branch-map JSON file.

Read:
- `branch-map-format.md` for the input structure
- `example-branch-map.json` for an example
...
1. write the branch data to JSON using the format in `branch-map-format.md`
...
- `branch-map-format.md` for the JSON structure
- `example-branch-map.json` for a starter example
- `state-labels.md` for recommended branch/node labels
